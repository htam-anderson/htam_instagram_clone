...
@Composable
fun BottomNavigationMenu(selectedIte: BottomNavigationItem, navController: NavController) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .wrapContentHeight()
        ...
        for (item in BottomNavigationItem.values()) {
            Image(
                ...
                colorFilter = if (item == selectedIte) ColorFilter.tint(Color.Black) else ColorFilter.tint(Color.Gray)
            )
        }
